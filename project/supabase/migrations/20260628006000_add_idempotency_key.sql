-- Add idempotency_key column to orders table if not exists
ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS idempotency_key TEXT UNIQUE DEFAULT NULL;

-- Recreate the place_order Postgres function to support and verify the idempotency key
...
BEGIN
  ...
  -- 1. Check for existing order with this idempotency key to prevent double submits
  ...
  -- 6.5 Validate that client-provided prices match the server-calculated prices to prevent price tampering
  ...
  -- 7. Insert order into public.orders using secure server-side values
  ...
  -- 10.5. Increment coupon usage count atomically if a coupon was used
  ...
  -- 11. Clear user cart items atomically for the current tenant only
