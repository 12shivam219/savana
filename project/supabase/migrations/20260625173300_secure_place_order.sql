-- PostgreSQL function to recalculate financial values on the server and prevent client-side price manipulation
...
BEGIN
  ...
  -- 2. First pass: Recalculate subtotal and validate/lock stock
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    v_variant_id := (v_item->>'variant_id')::uuid;
    v_quantity := (v_item->>'quantity')::integer;

    -- Lock variant row and check stock, and also fetch product price details
  ...
  -- 5. Insert order into public.orders using secure server-side values
  ...
  -- 8. Calculate and allocate loyalty points earned based on recalculated subtotal
